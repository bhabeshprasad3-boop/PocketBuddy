...
const App = () => {
  const [isDarkMode, setIsDarkMode] = useState(() => JSON.parse(localStorage.getItem('pocketTheme')) ?? true);
    ...
    <div className={`min-h-screen pb-32 transition-colors duration-500 ${bgClass} ${textClass}`}>
      ...
      <div className='max-w-5xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8'>
        ...
        <div className='lg:col-span-2 space-y-6'>
          ...
          <GoalTracker isDarkMode={isDarkMode} />
        </div>

        {/* RIGHT COLUMN */}
        <div className='lg:col-span-1'>
          <DailyTracker isDarkMode={isDarkMode} />
      ...
      {/* FAB */}
      <button onClick={() => setIsModalOpen(true)} className='fixed bottom-8 right-8 w-16 h-16 bg-gradient-to-tr from-emerald-500 to-cyan-500 text-white rounded-full shadow-[0_0_40px_-10px_rgba(16,185,129,0.5)] flex items-center justify-center transition-all hover:scale-110 active:scale-95 z-40 border border-white/20'><IoAdd size={32} /></button>
      
      {/* Modal with Voice Feature */}
      <AddExpenseForm isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} isDarkMode={isDarkMode} />
    </div>
  )
}
export default App;